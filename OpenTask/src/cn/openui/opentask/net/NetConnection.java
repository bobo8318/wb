...
public class NetConnection extends AsyncTask<Integer, Integer, String> {
	...
	/*public NetConnection(final String url, final HttpMethod method,final SuccessCallback success,final FailCallback fail, final String ...kvs){
		AsyncTask netTask = new AsyncTask<Void, Void, String>(){
			protected String doInBackground(Void ...arg0){
				StringBuffer sb = new StringBuffer();
				for(int i=0;i<kvs.length;i++){
					sb.append("/").append(kvs[i]);
				}
				try {
					URLConnection uc;
					switch(method){
					case POST:
						...
						break;
					default:
						uc = new URL("http://www.openui.cn/api/login/123/123/123").openConnection();
						break;
					}
